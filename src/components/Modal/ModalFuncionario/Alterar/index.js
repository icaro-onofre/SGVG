          ...
          <div className="flex flex-col items-center justify-center w-1/2 bg-white rounded-xl z-20 pt-5 pb-5">
            ...
            <div className="flex flex-col space-y-5 h-90 mt-8 ">
              <div className="flex flex-row space-x-5">
                <Input
                  placeholder={loading ? 'Loading...' : selectedFuncionarioDataFiltered[0].nome}
                  onChange={(e) => setNome(e.target.value)}
                />
                <Input
                  placeholder={loading ? 'Loading...' : selectedFuncionarioDataFiltered[0].telefone}
                  onChange={(e) => setTelefone(e.target.value)}
                />
              </div>
              <div className="flex flex-row space-x-5">
                <Input
                  placeholder={loading ? 'Loading...' : selectedFuncionarioDataFiltered[0].email}
                  onChange={(e) => setEmail(e.target.value)}
                />
                <Input
                  placeholder={loading ? 'Loading...' : selectedFuncionarioDataFiltered[0].cpf}
              ...
              <div className="flex flex-row space-x-5">
                <LocalizationProvider dateAdapter={AdapterDayjs}>
                  <DemoContainer components={['DatePicker']}>
                    <DatePicker
                      label={loading ? 'Loading...' : selectedFuncionarioDataFiltered[0].data_nasc}
