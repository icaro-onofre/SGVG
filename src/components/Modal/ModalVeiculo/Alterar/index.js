      ...
      {!foldVeiculoAlterar ? (
        ...
        <div className=" absolute w-screen h-screen bg-black/[0.85] z-20 inset-0 flex items-center justify-center transition duration-100 ease-in">
          ...
          <div className="flex flex-col items-center justify-center w-1/2 bg-white rounded-xl z-20 pt-5 pb-5">
            <h1 className="text-2xl font-bold ml-5 mt-1 self-start">{loading ? <Skeleton /> : 'Editar veiculo'}</h1>
            <div className="flex flex-col space-y-5 h-90 mt-8 ">
              <div className="flex flex-row space-x-5">
                <Input
                  placeholder={loading ? 'Loading...' : selectedVeiculoDataFiltered[0].categoria}
                  onChange={(e) => setCategoria(e.target.value)}
                />
                <Input
                  placeholder={loading ? 'Loading...' : selectedVeiculoDataFiltered[0].cor}
                  onChange={(e) => setCor(e.target.value)}
                />
              </div>
              <div className="flex flex-row space-x-5">
                <Input
                  placeholder={loading ? 'Loading...' : selectedVeiculoDataFiltered[0].clienteId}
                  onChange={(e) => setCliente(e.target.value)}
                />
                <Input
                  placeholder={loading ? 'Loading...' : selectedVeiculoDataFiltered[0].modelo}
                  onChange={(e) => setModelo(e.target.value)}
                />
              </div>
              <div className="flex flex-row space-x-5">
                <Input
                  placeholder={loading ? 'Loading...' : selectedVeiculoDataFiltered[0].placa}
                  onChange={(e) => setPlaca(e.target.value)}
                />
              </div>
              <div className="self-end">
                <Button
                  value="Editar Veiculo"
                  onClick={() => {
                    handleSubmit();
                  }}
                />
